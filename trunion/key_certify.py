...
def certificate(privkey, expiry_timestamp, price_limit, issuer=None,
    ...
    if issuer is None:
        issuer = crypto.KEYSTORE.cert_data['kid']
    ...
    certificate = {
        ...
        "key":
            [
            {
                "alg": "RSA",
                "kid": keyid,
                "mod": jwt.base64url_encode(pubKey[1]),
                "exp": jwt.base64url_encode(pubKey[0])
                }
            ],
        "nbf": long(issued_at),
...
def self_signed(bits, expires, limit, issuer):
    rsaObj = M2Crypto.RSA.gen_key(bits, 0x10001, NoOp)

    cert = certificate(rsaObj, expires, limit, issuer)

    # Self certify
    return (rsaObj.as_pem(None), jwt.encode(cert, rsaObj, 'RS256'))
